...
//echo $_GET['start'];die;
	...
	$rsAdmin = $cms->db_query("select customer_name, quotation_number, panel_model, panel_wattage, panel_count, inverter_type, inverter_type2, proposal_mms_cost, charger_name, battery_name, solar_margin, solar_margin_kr, charger_margin, charger_margin_kr from #_leads where id='".$_REQUEST['pid']."'");
	...
	@extract($arrAdmin);
		...
		$txt .= "4. Solar margins (%):".$solar_margin."\n";
		$txt .= "5. Solar margins (kr):".$solar_margin_kr."\n";
		
		if($charger_name){
		$txt .= "6. EV charger:".$charger_name.", Quantity:1\n";
		$txt .= "7. EV Charger margins (%):".$charger_margin."\n";
		$txt .= "8. EV Charger margins (kr):".$charger_margin_kr."\n";
		}
		if($battery_name){
		$txt .= "9. Battery:".$battery_name.", Quantity:1\n";
		}
