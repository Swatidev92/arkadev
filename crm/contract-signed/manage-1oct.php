...
<?php 
	...
	if(isset($_REQUEST['submt'])){
		...
		$pagesize = intval($pagesize)==0?$pagesize=DEF_PAGE_SIZE:$pagesize;
		$columns = "select * ";
		$sql = " from #_leads where 1=1 AND lead_id>0 AND is_deleted=0 AND status=4 ";
		$order_by == '' ? $order_by = 'post_date' : true;
		$order_by2 == '' ? $order_by2 = 'asc' : true;
		$sql_count = "select count(*) ".$sql; 
		$sql .= "order by $order_by $order_by2 ";
				...
				<table class="table table-striped1 table-bordered1 table-hover">
					...
					<tbody>
						<?php if($reccnt){ 
							...
							$project_created = $cms->db_query("SELECT cust_id FROM #_customer_project where cust_id=$id ");
							if($project_created->num_rows==0){
						?>  
						<tr>
