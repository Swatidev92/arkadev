				...
				<table class="table table-striped table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<?php 
									$stepsQry=$cms->db_query("SELECT content_title,id FROM #_help_content where helpcat_id=$id  AND is_deleted=0 ");
							...
							<a href="<?=SITE_PATH_ADM."help-content/"."?mode=add&courseId=".$id?>">Add New Content</a><hr>
								...
								<ul>
									...
									<li><a href="<?=SITE_PATH_ADM."help-content/"."?mode=edit&courseId=".$stepsRes['id']?>"><?=$stepsRes['content_title'];?></a></li>
									<?php } ?>
								</ul>
							</td>
							<?php } //else{?>
...
<script type="text/javascript">
	function updateStatus(id,current_status){
		$.ajax({
			...
			data:"id="+id+"&status="+current_status+"&table_name=help_category",
			method:"post"
		})
	}
</script>