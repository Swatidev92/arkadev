...
// echo $_GET['start'];die;
	...
	// echo $lead_id;die;
	$otherDetailsQry = $cms->db_query("SELECT * FROM #_other_details where parent_id= '".$lead_id."'");
	// echo "1";die;
	...
	$total_margin = 0;
		...
		// s:mk-19
		if($proposal_type == 1 || $proposal_type == 8 || $proposal_type == 9 || $proposal_type == 11 ){

				if($otherDetailsArr['cable_len_inv']!="" || $otherDetailsArr['cable_len_inv']!=null ){
				$txt .= "Cable length in meter for inverter:".$otherDetailsArr['cable_len_inv']."\n";}
		}
		if($proposal_type == 5 || $proposal_type == 8 || $proposal_type == 9 ){	
				if($otherDetailsArr['cable_len_ev']!="" || $otherDetailsArr['cable_len_ev']!= null){
					$txt .= "Cable length in meter for ev:".$otherDetailsArr['cable_len_inv']."\n";}
				//$txt .= "\t ";
