    ...
    <div class="col-sm-12">
		<div class="white-box">
			<?php
				...
				$lnk3="#";
				if($leadid>0){
					$lnk1="?mode=add-proposal-newgr&start=&t=prop_details&leadid=".$leadid."&id=".$pid;
					$lnk3="?mode=add-proposal-newgr&start=&t=other_details&leadid=".$leadid."&id=".$pid;
					$lnk2="?mode=add-proposal-newgr&start=&t=roof_details&leadid=".$leadid."&id=".$pid;
				}					
				...
				<div role="tabpannel" class="tab-pane <?php echo $active1;?>" id="proj_info">
					...
					<div class="form-group col-md-3 show-solar-margin" style="<?=$stylesm?>">
						...
						<input type="number" class="form-control" name="solar_margin" value="<?=($leadid!='' && $pid!='')?($solar_margin?$solar_margin:$smargin):$smargin?>" <?=$_SESSION["ses_adm_role"]==1?'':''?> min="<?=($_SESSION["ses_adm_role"]==1)?-50:$smargin?>">
					</div>
					<?php if($proposal_type==2 || $proposal_type==3 || $proposal_type==5 || $proposal_type==7 || $proposal_type==8 || $proposal_type==9){
						$stylem = '';
					}else{	
					...
					<div class="form-group col-md-3 show-charger-margin" style="<?=$stylem?>">
						...
						<input type="number" class="form-control" name="charger_margin" value="<?=($leadid!='' && $pid!='')?($charger_margin?$charger_margin:$cmargin):$cmargin?>" <?=$_SESSION["ses_adm_role"]==1?'':''?> min="<?=($_SESSION["ses_adm_role"]==1)?-10:$cmargin?>">
					</div>
					<?php  if($proposal_type==3 || $proposal_type==4 || $proposal_type==6 || $proposal_type==7 || $proposal_type==8 || $proposal_type==10 || $proposal_type==11){
						$stylebm = '';
					}else{	
					...
					<div class="form-group col-md-3 show-battery-margin" style="<?=$stylebm?>">
						...
						<input type="number" class="form-control" name="battery_margin" value="<?=($leadid!='' && $pid!='')?($battery_margin?$battery_margin:$bmargin):$bmargin?>" <?=$_SESSION["ses_adm_role"]==1?'':''?> min="<?=($_SESSION["ses_adm_role"]==1)?-10:$bmargin?>">
					</div>		
					<?php if($obj_btmrg[0]->status==1){
						$bmargin = $obj_btmrg[0]->margin;
						$binfo = '';
