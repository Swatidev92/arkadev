		...
		<div class="white-box">
			<div class="form-group">
				<?php if(in_array(4,$act_arr) || $_SESSION["ses_adm_id"]==1){ ?>
					<!-- 1st version: <a href="<?//=SITE_PATH_ADM.CPAGE.'/?mode=add-proposal&start='.$_GET['start'].'&t=prop_details&leadid='.$pid?>" class="ub">
				...
				<table class="table table-striped1 table-hover1 table-bordered1">
					<thead>
						<tr>
							...
							<th>Allowed Green Rebate</th>
							<th>Percentage Green Rebate</th>
					...
					<tbody>
						...
						<tr class="clickable-row" style="<?=$row_bg?>">
							...
							$clr1='';
							$clr2='';
							$v1=round($total_green_rebate_kr);
							$v2 = round($percentg_gr_rebate);
								if($v2>$v1)
								{
									$clry = "";
									$clrg = "green";
							...
							?>
							<td class="table-center text-nowrap" style="color:<?=$clrg?>"><?=amount_format_proposal(round($total_green_rebate_kr))?></td>
							
							<td class="table-center text-nowrap" style="color:<?=$clry?>"><?=amount_format_proposal(round($percentg_gr_rebate))?></td>
