		...
		if($arr_ids){
			if($_POST['username']!=''){ 
				...
				foreach($str_adm_idsArr as $lid){
					...
					$statusLeadQry = $cms->db_query("SELECT to_user, to_admin FROM #_lead_type_status where constant=2 and lead_type=1 ");
	...
	*/
	...
	if(!empty($_GET)){
		...
		if(!empty($_REQUEST['search_city'])){
			$search_city = $cms->escape_string($_REQUEST['search_city']);
			$search_city2 = trim($_REQUEST['search_city']);
			$conditionsQr .= " AND l.city like '%$search_city2%'";
		}
		...
		$columns = "SELECT l.id, l.status, l.created_date, l.post_date, l.customer_name, l.proposal_address, l.assigned_to, l.form_type, l.post_by, l.city ";
		$sql = " FROM #_leads l WHERE 1=1 AND l.is_deleted=0 AND l.lead_id=0 ".$conditionsQr." $adminRole ";
		$order_by == '' ? $order_by = 'l.created_date' : true;
		...
		$start = intval($_GET['start']);
		$pagesize = intval($pagesize)==0?$pagesize=DEF_PAGE_SIZE:$pagesize;
		$columns = "SELECT l.id, l.status, l.created_date,l.post_date, l.customer_name, l.proposal_address, l.assigned_to, l.form_type, l.post_by, l.city,l.update_date ";
					...
					<div class="panel">
						...
						<div class="panel-collapse collapse <?=$expanded=='true'?'in':''?>" id="exampleCollapseDefaultThree1" aria-labelledby="exampleCollapseDefaultThree1" role="tabpanel">
							<div class="panel-body">
								...
								<div class="form-group col-sm-3">
									<input type="text" name="search_city" id="search_city" value="<?=$search_city?>" class="form-control" id="search_city" placeholder="Search City" data-fv-regexp="true">
									<div class="help-block with-errors"></div>
								</div>
								...
								<div class="clearfix"></div>
								<?php if(($_SESSION["ses_adm_role"]=="1" || (in_array(3,$act_arr))) && $_SESSION['ses_adm_id']!=7){ ?>
								<div class="form-group col-sm-4">
					...
					<tbody>
						...
						<tr class="clickable-row" style="<?=$row_bg?>">
							...
							<td class="text-nowrap">
								...
								$lead_priority = $cms->getSingleResult("SELECT dimensioning_priority FROM #_lead_tracker where lead_id=$id AND dimensioning_priority<9999 ");
								?>
							...
							<td class="table-center text-nowrap"><?=$city?></td>
							<td class="table-center text-nowrap"><?=$leadsStatusArr[$status]?>/<?=$current_status_date?></td>
							<td class="table-center text-nowrap"><?=($lead_priority<9999 && $lead_priority!='')?$lead_priority:'NA'?></td>
