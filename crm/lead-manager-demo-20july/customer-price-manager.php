...
if($cms->is_post_back()){ 
	if($_FILES["banner"]["name"]){ 
		$_POST["banner"] = uploadImage("banner","about");
	}
	if($_FILES["sec2_icon1"]["name"]){ 
		$_POST["sec2_icon1"] = uploadImage("sec2_icon1","about");
	}
	if($_FILES["sec2_icon2"]["name"]){ 
		$_POST["sec2_icon2"] = uploadImage("sec2_icon2","about");
	}
	if($_FILES["sec2_icon3"]["name"]){ 
		$_POST["sec2_icon3"] = uploadImage("sec2_icon3","about");
	}
	if($_FILES["sec2_icon4"]["name"]){ 
		$_POST["sec2_icon4"] = uploadImage("sec2_icon4","about");
	}
	if($_FILES["sec2_icon5"]["name"]){ 
		$_POST["sec2_icon5"] = uploadImage("sec2_icon5","about");
	}
	
	
	$cms->sqlquery("rs","about",$_POST,'id',1);
...
$rsAdmin=$cms->db_query("select * from #_about where id='1'");
								...
								<div class="panel-collapse collapse in" id="exampleCollapseDefaultOne" aria-labelledby="exampleHeadingDefaultTwo" role="tabpanel">
									<div class="panel-body">
										<div class="col-sm-6"> 
											...
											<input type="file" name="banner" id="input-file-max-fs" class="dropify" data-max-file-size="1M" data-height="150" <?php if($banner AND file_exists(FILES_PATH.'about/'.$banner)){ ?> data-default-file="<?=SITE_PATH?>uploaded_files/about/<?=$banner?>" <?php }else{ ?> required <?php } ?> />
										...
										<div class="col-sm-6">
											...
											<input type="text" name="banner_text" value="<?=$banner_text?>" class="form-control" id="banner_text" placeholder="Banner Text" data-fv-regexp="true" data-error="Please enter value"> 
										</div>
										<div class="clearfix"></div>
									</div>
								</div>
