...
if($cms->is_post_back()){	
	...
	if($old_status!=$status && $old_status!=''){
		...
		if($_POST['old_status']==1 && $_POST['status']==2 && !empty($_POST["assigned_to"])){
			...
			$AssignPOSTS["activity_message"] = 'Customer '.$_POST['customer_name'].' '.$uniqueLeadId.'assigned to '.$assignedTo;
...
<div class="row">
    <div class="col-sm-12">
        ...
        <div class="white-box">			
			...
			<?php if(!in_array($_SESSION["ses_adm_id"],$dimensioningUserArr) && $dimensioning_user){
				$dstyle = "";
			}else{
				$dstyle = "display:none";
			}?>
			<div class="form-group col-sm-4" id="show-dimensioning-user" style="<?=$dstyle?>">							
			...
			<div class="form-group col-sm-12">
				...
				<a href="<?=SITE_PATH_ADM.CPAGE?>" class="" ><button type="button" class="btn back-btn">Back</button></a>
			</div>				
			<div class="clearfix"></div>
		</div>
	</div>
