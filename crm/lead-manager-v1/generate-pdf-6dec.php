...
//echo $_GET['start'];die;
	...
	@extract($arrAdmin);
		...
		$txt .= "EV Charger margins (%):".$charger_margin."\n";
		$txt .= "EV Charger margins (kr):".amount_format_proposal(round($charger_margin_kr))."\n";
		$txt .= "EV Charger VAT (kr):".amount_format_proposal(round($charger_vat_kr))."\n";
		if($proposal_customer_type==1){
		$txt .= "EV Charger Green rebate (kr):".amount_format_proposal(round($charger_green_rebate_kr))."\n";
		}
		}
		if($battery_name){
 ...
 $pdf->SetTextColor(0, 0, 0);
	...
	//$pdf->setTextShadow(array('enabled'=>true, 'depth_w'=>0.2, 'depth_h'=>0.2, 'color'=>array(196,196,196), 'opacity'=>1, 'blend_mode'=>'Normal'));
		...
		*/
		...
		if($proposal_type==3){
			$html5=file_get_contents(SITE_PATH."inc2/proposal-format5-case3-battery.php", false, $context);
			$pdf->writeHTMLCell(0, 0, '', '0', $html5, 0, 1, 0, true, '', true);
			$pdf->AddPage();
		}
		if($proposal_type==4){
