...
//echo $_GET['start'];die;
	...
	$total_margin = 0;
		...
		if($proposal_type==1){
			$file_name = '(Solar-'.$panel_count.')';
		}
		// solar and charger and battery
		if($proposal_type==3 || $proposal_type==8){
			$file_name = '(Solar-'.$panel_count.'-EVcharger-'.$charger_name.'-Battery-'.$battery_name.')';
		}
		// solar and charger
		if($proposal_type==2 || $proposal_type==9){
			$file_name = '(Solar-'.$panel_count.'-EVcharger-'.$charger_name.')';
		}
		// solar and battery
		if($proposal_type==4 || $proposal_type==10){
			$file_name = '(Solar-'.$panel_count.'-Battery-'.$battery_name.')';
		}
		// battery and charger
		if($proposal_type==7){
			$file_name = '(Battery-'.$battery_name.'-EVcharger-'.$charger_name.')';
		}
		// only charger
		if($proposal_type==5){
			$file_name = '(EVcharger-'.$charger_name.')';
		}
		// only battery
		if($proposal_type==6){
			$file_name = '(Battery-'.$battery_name.')';
		}
