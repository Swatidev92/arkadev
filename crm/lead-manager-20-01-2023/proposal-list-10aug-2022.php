						...
						<tr class="clickable-row" style="<?=$row_bg?>">
							...
							<td class="text-nowrap">
								...
								<?php $checked=($status=="4"?"checked":""); 
									$old_status = $status;
								?>
								<input data-size="small" type="checkbox" onChange="updateStatus('<?=$id?>','<?=$status?>','<?=$old_status?>')" <?=$checked?> class="js-switch"  data-toggle="tooltip" title="Change Status" data-color="#99d683" data-secondary-color="#f96262" />
								<?php }else{?>
...
function updateStatus(id,current_status,old_status){
	$.ajax({
		url:"<?=SITE_PATH_ADM.CPAGE?>/ajaxChangeStatus.php",
		data:"id="+id+"&status="+current_status+"&old_status="+old_status,
		method:"post",
		beforeSend:function(){
