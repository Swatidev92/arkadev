				...
				<table class="table table-striped1 table-hover1 table-bordered1">
					<thead>
						<tr>
							...
							<th class="text-nowrap">Contact Signed</th>
						...
						<tr class="clickable-row" style="<?=$row_bg?>">
							...
							<td class="text-nowrap">
								...
								<?php 
								 ...
								 <br>
								 <br>
								 <a href="<?=SITE_PATH_ADM.CPAGE?>/generate-pdf2.php?pid=<?=$propid?>" title="Generate Proposal"><i class="fa fa-download" aria-hidden="true"></i></a>&nbsp;&nbsp;
							...
							<td class="text-nowrap">
								...
								<?php $checked=($status=="4"?"checked":""); 
									$old_status = $cms->getSingleResult("SELECT new_status FROM #_lead_tracker where lead_id=$pid ");
								?>
								<input data-size="small" type="checkbox" onChange="updateStatus('<?=$id?>','<?=$pid?>','<?=$status?>','<?=$old_status?>')" <?=$checked?> class="js-switch"  data-toggle="tooltip" title="Change Status" data-color="#99d683" data-secondary-color="#f96262" />
								<?php }else{?>
