...
<div id="uplaod">
	...
	<div class="form-group col-sm-12">
		<?php $docfileQry = $cms->db_query("SELECT * FROM #_uploads where lead_id=$pid AND file_type=1 ");
		...
		<div class="col-sm-6">
			<div class="upload-box">
				...
				<hr>
				<?php }else{
				$docfileQry = $cms->db_query("SELECT * FROM #_uploads where lead_id=$pid AND file_type=1 AND file_title='".$doc_name."' order by id ");
				$fileRes = $docfileQry->fetch_array();
				?>
				<div class="form-group col-sm-12">
					<div id="<?=$fileRes['id']?>">
						<h3><?=$fileRes['file_title']?$fileRes['file_title'].' (Dimensioning)- ':''?></h3>
						<?=$fileRes['file_upload']?> <a href="javascript:void(0)" class="remove_icon" onClick="remove_uploaded_file('<?=$fileRes['id']?>','<?=$fileRes['file_upload']?>')" data-toggle="tooltip" data-original-title="Delete File"><img src="<?=SITE_PATH_ADM?>images/close-btn.png"></a> <a href="<?=SITE_PATH?>uploaded_files/uploads/<?=$fileRes['file_upload']?>" download>View</a></div>
				</div>
				<div class="clearfix"></div>
				<hr>
				<?php } } ?>
				...
				<div class="mms-wrapper">					
					<div class="row">
						...
						<div class="clearfix"></div>
						<div class="col-md-12">
						<?php $docfileQry = $cms->db_query("SELECT * FROM #_uploads where lead_id=$pid AND file_type=1 AND file_title='".$doc_name."' ");
				...
				<div class="clearfix"></div>		
				<?php } ?>
				<?php $docfileQry = $cms->db_query("SELECT * FROM #_uploads where lead_id=$pid AND file_type=1 AND file_title!='Solar Report' AND file_title!='MMS Report Roof' AND file_title!='BOM MMS Roof' AND file_title!='Föranmälan' ");
		...
		<div class="col-sm-6">
			<div class="upload-box">
				...
				<?php if($pic_id==1){
				$picfileQry = $cms->db_query("SELECT * FROM #_uploads where lead_id=$pid AND file_type=2 AND file_title='".$pic_name."' ");
				...
				<div class="form-group col-sm-12">
					<div class="row" id="<?=$picfileRes['id']?>">
						<div class="form-group col-sm-10">
							...
							<img src="<?=SITE_PATH.'uploaded_files/uploads/'.$picfileRes['file_upload']?>" width="100%">
						</div>
				...
				<div class="panel-roof-wrapper">					
					<div class="row">
						...
						<div class="clearfix"></div>
						<div class="col-md-12">
						<?php $picfileQry = $cms->db_query("SELECT * FROM #_uploads where lead_id=$pid AND file_type=2 AND file_title='".$pic_name."' order by id ");
				...
				<div class="row" id="<?=$otherPicfileRes['id']?>">
					<div class="form-group col-sm-8">
						...
						<img src="<?=SITE_PATH.'uploaded_files/uploads/'.$otherPicfileRes['file_upload']?>" width="100%">
					</div>
					<div class="form-group col-sm-3">
	...
	<!--<div class="form-group col-sm-12">
		<?php $docfileQry = $cms->db_query("SELECT * FROM #_uploads where lead_id=$pid AND file_type=1 order by id ");
		if($docfileQry->num_rows>0){ ?>
			<ul>
