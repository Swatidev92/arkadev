...
<?php 
	...
	if(isset($_REQUEST['submt'])){
		...
		$sql = " from #_leads l LEFT JOIN #_customer_project p on l.id=p.cust_id where 1=1 AND l.lead_id>0 AND l.is_deleted=0 AND l.status=4 AND p.status!=7 AND p.status!='' $conditionsQr ";		
		...
		$pagesize = intval($pagesize)==0?$pagesize=DEF_PAGE_SIZE:$pagesize;
		$columns = "select p.status as pstatus, l.* ";
		$sql = " from #_leads l LEFT JOIN #_customer_project p on l.id=p.cust_id where 1=1 AND l.lead_id>0 AND l.is_deleted=0 AND l.status=4 AND p.status!=7 AND p.status!='' ";
		$order_by == '' ? $order_by = 'l.post_date' : true;
		$order_by2 == '' ? $order_by2 = 'asc' : true;
		$sql_count = "select count(*) ".$sql; 
		$sql .= "order by $order_by $order_by2 ";
