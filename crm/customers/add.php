...
if($cms->is_post_back()){ 
	...
	if($pid){
		...
		$cms->db_query("UPDATE #_leads SET aggrement_note='".$_POST['aggrement_note']."' WHERE id=$lead_id ");
...
if(isset($pid)){
	$asAdmin=$cms->db_query("select * from #_leads where id='".$lead_id."'");
	$asrrAdmin=$cms->db_fetch_array($asAdmin);
//@extract($arrAdmin);
}
// if(isset($pid)){
// 	if(empty($aggrement_note)){
// 		$rsAdmin1=$cms->db_query("select * from #_leads where lead_id='".$lead_id."'");
// 		$arrAdmin1=$cms->db_fetch_array($rsAdmin1);
// 		@extract($arrAdmin1);
// 		$cms->db_query("UPDATE #_leads SET aggrement_note='".$aggrement_note."' WHERE id=$pid ");
// 	}
// }
// echo "<pre>";
// print_r($arrAdmin);
    ...
    <div class="col-sm-12">
        <div class="white-box">			 
			...
			<div class="form-group col-sm-6">
				...
				<textarea name="aggrement_note" class="form-control" id="aggrement_note" data-fv-regexp="true" data-error="Please enter value in correct format." maxlength="250"><?=$asrrAdmin['aggrement_note']?></textarea>
