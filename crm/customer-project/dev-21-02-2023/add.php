...
if($cms->is_post_back()){ 
	...
	*/
	...
	if(isset($_POST['mail'])){
		...
		// $headers = "BCC: mukund.bluedigital@gmail.com";
        ...
        if($total_rec>0){
            ...
            //echo $roof_details_count;die;
                ...
                for($i=0;$i<=$roof_details_count;$i++){
					...
					// if($_POST['roofing_material'][$i] == 'TrapetsKorrugerad Plåt (screws/ skruv)' || $_POST['roofing_material'][$i] == 'TrapetsKorrugerad Aluminium (rivets/popnit)' || $_POST['roofing_material'][$i] ==  'VågKorrugerad Plåt (rivet/popnit)' || $_POST['roofing_material'][$i] == 'VågKorrugerad Aluminium (rivets/popnit)' || $_POST['roofing_material'][$i] == 'Plegel tak (rivets/popnits)' ){
					// $roofDetails['roof_thickness'] =$_POST['roof_thickness'][$i];
					// $roofDetails['roof_material'] =$_POST['roof_material'][$i]; }
			...
			if(count($_POST['total_panel'])!=0 || count($_POST['roofing_material'])!=0 || count($_POST['roof_support'])!=0 ||  count($_POST['roof_angle'])!=0 )
			{
				...
				// $roofDetails['roof_thickness'] =$_POST['roof_thickness'];
				// $roofDetails['roof_material'] =$_POST['roof_material'];
...
$leadsQry = $cms->db_query("SELECT * FROM #_leads where id = '".$cust_id."'");
$leadsArr = $leadsQry->fetch_array();
//print_r($leadsArr['panel_count']);die;
// e:mk-19	
?>
<style>
	.arrow-steps .step {
    ...
    <div class="col-sm-12">
        <div class="white-box">
			...
			<div class="arrow-steps clearfix">
				<?php 
				$fr_arr=array(1,8,2,3);
				$inst_arr=array(4,5,6,9);
				$arch_arr=array(7,10);
				foreach($proposalStatus as $pkey=>$pval){
					
					if(in_array($pkey,$fr_arr))
					{
						if($pkey==$status){
						$current = 'progress_st';
						$carr = $fr_arr;
						...
						$frml_str.='<div id="id'.$pkey.'" class="step '.$current.'"> <span>'.$pval.'</span> </div>';
					}
					if(in_array($pkey,$inst_arr))
					{
						if($pkey==$status){
						$current = 'progress_st';
						$carr = $inst_arr;
						...
						$inst_str.='<div id="id'.$pkey.'" class="step '.$current.'"> <span> '.$pval.'</span> </div>';
					}
					if(in_array($pkey,$arch_arr))
					{
						if($pkey==$status){
						$current = 'progress_st';
						$carr = $arch_arr;
						...
						$arch_str.='<div id="id'.$pkey.'" class="step '.$current.'"> <span> '.$pval.'</span> </div>';
					}
				?>
				<!--<div class="step <?=$current?>"> <span> <?=$pval?><?=$pkey?></span> </div>-->
				...
				<div class="row">
					...
					<div class="col-sm-1">
					<b align="center">Installation</b>
					</div>
					<div class="col-sm-11">
					<?=$inst_str?>
					</div>
					<div class="col-sm-1">
					<b align="center">Handover</b>
					</div>
					<div class="col-sm-11">
					<?=$arch_str?>
					</div>
			...
			<div class="tab-content" style="margin-top:0px;">
				...
				<div role="tabpanel" class="tab-pane <?php echo $active1;?>" id="proj_info">
					...
					<div class="form-group col-sm-4">
						<label for="facade_meter_location" class="control-label" style="<?=$sales_field_color?>">Väggmaterial  växelriktare</label>
						<input type="text" name="vagmaterial" value="<?=$vagmaterial?>" class="form-control purple-field" id="vagmaterial" data-fv-regexp="true" data-error="Please enter valid Name">
						<div class="help-block with-errors"></div>
					</div>
					<div class="clearfix"></div>
					<div class="form-group col-sm-6">
                ...
                <div role="tabpanel" class="tab-pane <?php echo $active6;?>" id="roof_details">
                    ...
                    <!-- S:Edit-->
                        <div class="row" id="btrow<?=$i?>">
                            ...
                            <div class="form-group col-md-3" id="roof">
                                <label for="roofing_material" class="control-label">Roof Type</label>
                                <select class="form-control" id="roofing_material<?=$i?>" name="roofing_material[<?=$i?>]" <?= $readonly_field ?> onchange="roof_material()">
							...
							<!-- <div id="roof_related<?=$i?>">
								<div class="form-group col-md-1" id="thickness">
									<label for="roof_thickness" class="control-label">Thickness</label>
									<input type="text" class="form-control" name="roof_thickness[<?=$i?>]" id="roof_thickness<?=$i?>" value="<?= $roof_thickness ?>" <?= $readonly_field ?>>
								</div>
								<div class="form-group col-md-1" id="material">
									<label for="roof_material" class="control-label">Material</label>
									<input type="text" class="form-control" name="roof_material[<?=$i?>]" id="roof_material<?=$i?>" value="<?= $roof_material ?>" <?= $readonly_field ?>>
								</div>
							</div> -->
                        ...
                        <div class="row" id="btrow<?=$i?>">
                            ...
                            <div class="form-group col-md-3" id="roof">
                                <label for="roofing_material" class="control-label">Roof Type</label>
                                <select class="form-control" id="roofing_materiall" name="roofing_material[<?=$i?>]" <?= $readonly_field ?>>
                                    <option value="">Select Roof type</option>
                                    <?php $roofTypePriceArr = json_decode($customerPriceArr["roof_type_price"], true);
                                    foreach ($roofTypePriceArr as $rkey => $rvalue) {
                                    if ($rvalue["rfstatus"] == 1) {
							...
							<!-- <div id="roof_related<?=$i?>">
								...
								<div class="form-group col-md-1" id="material">
									<label for="roof_material" class="control-label">Material</label>
									<input type="text" class="form-control" name="roof_material[<?=$i?>]" id="roof_material<?=$i?>"  <?= $readonly_field ?>>
								</div>
							</div> -->
							<div class="form-group col-md-2">
                                <label for="roof-support" class="control-label">Roof Support</label><br>
				...
				<!-- s:inventory mk-19 -->
				<div role="tabpanel" class="tab-pane <?php echo $active7;?>" id="inventory">
						<?php $i=0; ?>
						...
						<table class="table table-bordered">
							...
							<?php if($customerProjectArr['ev_charger']!="" || $customerProjectArr['ev_charger']!=null ) {?>
							<tr>
							...
							<tr>
								<td><?php $i=$i+1; echo $i;?></td>
								<td>Överspänningsskydd DC</td>
								<td>-</td>
								<td id="surg_dc_show">
									<span id="surg_dc_hide"><?php if($customerProjectArr['surge_protc_dc']!="" || $customerProjectArr['surge_protc_dc']!=null){ echo $customerProjectArr['surge_protc_dc']; } else{ echo $otherDetailsArr['surge_protc_dc']; }?></span>
									<!-- <a class="pull-right" id="surg_dc_edit" onclick="surg_dc()"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td> -->
							</tr> 
							<?php } } ?>
							
							<?php if($leadsArr['proposal_type']== 5 ||$leadsArr['proposal_type']== 8||$leadsArr['proposal_type']== 9)  { if($otherDetailsArr['cable_len_ev']!="" || $otherDetailsArr['cable_len_ev']!=null ) {?>
							<tr>
								<td><?php $i=$i+1; echo $i;?></td>
								<td>Cable length between</td>
...
<script type="text/javascript">
    $(document).ready(function()
    {
		...
		// var roof = $('select#roofing_material' + x5).val();
		
		
		if(<?=$roof_details_count?> !== 0){ x5 = x5-1; }
		//else{ x5= x5+1; }
		...
		$('.list_add_button5').click(function()
	    {
			...
			$('select#roofing_material'+x5).change(function (){
				var ids= $(this).attr('id');
				var x2 = ids.charAt(ids.length-1);
				var end_x5= parseInt($("#total_rec").val());
				var roof = $('select#roofing_material'+x2).val();
					console.log(roof);
					if(roof == 'TrapetsKorrugerad Plåt (screws/ skruv)' || roof == 'TrapetsKorrugerad Aluminium (rivets/popnit)' || roof ==  'VågKorrugerad Plåt (rivet/popnit)' || roof == 'VågKorrugerad Aluminium (rivets/popnit)' || roof == 'Plegel tak (rivets/popnits)'){
					$('#roof_related' + x2).show(); 
					}else{ 
						$('#roof_related' + x2).hide();
					}
...
<script>
	$(document).ready(function(){
		$('#roof_related').hide();
		$('select#roofing_materiall').change(function(){
			var roof = $('select#roofing_materiall').val();
			console.log(roof);
			if(roof == 'TrapetsKorrugerad Plåt (screws/ skruv)' || roof == 'TrapetsKorrugerad Aluminium (rivets/popnit)' || roof ==  'VågKorrugerad Plåt (rivet/popnit)' || roof == 'VågKorrugerad Aluminium (rivets/popnit)' || roof == 'Plegel tak (rivets/popnits)'){
				$('#roof_related').show(); 
				}else{ 
...
<script>
	$(document).ready(function() { 
		var x1 = 0;
		var end_x1= parseInt($("#total_rec").val());
		// console.log(end_x1);
		while(x1 < end_x1){ 
		var roof = $('select#roofing_material' + x1).val();
		if(roof == 'TrapetsKorrugerad Plåt (screws/ skruv)' || roof == 'TrapetsKorrugerad Aluminium (rivets/popnit)' || roof ==  'VågKorrugerad Plåt (rivet/popnit)' || roof == 'VågKorrugerad Aluminium (rivets/popnit)' || roof == 'Plegel tak (rivets/popnits)'){
			$('#roof_related'+x1).show();
			}else{ 
				$('#roof_related'+x1).hide();
			}
			x1++;
		}
	});
	...
	function roof_material()
	{
		var x2 = 0; 
		var end_x2= parseInt($("#total_rec").val());
		while(x2 <end_x2){
			var roof = $('select#roofing_material'+x2).val();
			//console.log(x2);
			if(roof == 'TrapetsKorrugerad Plåt (screws/ skruv)' || roof == 'TrapetsKorrugerad Aluminium (rivets/popnit)' || roof ==  'VågKorrugerad Plåt (rivet/popnit)' || roof == 'VågKorrugerad Aluminium (rivets/popnit)' || roof == 'Plegel tak (rivets/popnits)'){
			...
			//var roof = $("#roofing_material[0]").val();
			//console.log(roof); 	
			x2++;
		}
