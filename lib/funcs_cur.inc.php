...
function hitcount($id){
	$as="aaa".$id;
	if($_SESSION[$as]==''){
		$this->mysqli->query("update ".tb_Prefix."auction set hits=(hits+1) where aid='".$id."'");
		$_SESSION[$as]='bdnow';
	}
}
...
function check_item_in_cat($catId)
{
	...
	$tempId="";
	while($arrCat=$rsCat->fetch_array())
	{
