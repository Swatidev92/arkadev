...
//echo $m1 = sendEmail('sneha.techblue@gmail.com', 'hi', 'Thankyou');die;
	...
	function getAllAgents(){
		...
		$agentQry = $cms->db_query("SELECT id, customer_name FROM #_users WHERE id!=1 AND role=3 AND status=1 AND is_deleted=0 ORDER BY customer_name ");
	...
	function get_customer_list($cid = 0) {
		...
		$cqr="SELECT * FROM #_leads WHERE status=4 AND is_deleted=0 AND customer_name!='' $adminRole order by customer_name ";
		$query1 = $cms->db_query($cqr);
		while($row = $query1->fetch_array()) {
			$current_id = $row['id']; 
			if($cid==$current_id)
