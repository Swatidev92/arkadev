...
//echo $m1 = sendEmail('sneha.techblue@gmail.com', 'hi', 'Thankyou');die;
	...
	function get_customer_list($cid = 0) {
		global $cms;
		$cqr="SELECT * FROM #_leads WHERE status=4 AND is_deleted=0 AND customer_name!='' order by customer_name ";
		$query1 = $cms->db_query($cqr);
		while($row = $query1->fetch_array()) {
			$current_id = $row['id']; 
			if($cid==$current_id)
	...
	$installerStatus = array("1"=>"New","2"=>"Approved","3"=>"Rejected","4"=>"Black List")
