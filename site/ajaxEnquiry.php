		...
		if($_POST["first_name"]!='' && $_POST["email"]!='' && $_POST["phone"]!=''){
			...
			foreach ($black_char as $url) {
				...
				if (strstr($_POST["first_name"], $url) !== FALSE) { 
					...
					$match=1;
				}if (strstr($_POST["city"], $url) !== FALSE) { 
					$match=1;
			...
			if($match==1){
			    ...
			    //new db connection
			    $cms_new = new DAL('localhost','arkaenergy_akausr21','S3wU8lmix*m[','arkaenergy_arkadb');
				$enquiry_id = $cms_new->sqlquery("rs","leads",$_POST);
