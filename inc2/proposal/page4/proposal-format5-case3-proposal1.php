...
$obj_ac = json_decode($cms->getSingleResult("SELECT content FROM #_customer_price_manager where field_const='ac_protect' "),true);
$obj_dc = json_decode($cms->getSingleResult("SELECT content FROM #_customer_price_manager where field_const='dc_protect'"),true);
		...
		<div class="block">
			...
			<table width="100%" cellpadding="0" cellspacing="0" border="0" id="backgroundTable" st-sortable="bigimage">
				<tbody>
					<tr>
						<td style="width: 100%;">
							<table class="" cellpadding="10" style="border-collapse: collapse; width: 100%; background-color:#000;" border="0">
								<tbody>
									<tr>
										<td valign="middle" style="text-align:left;font-size:18px;border:none; color:#fff;">
					...
					<tr>
						<td>
							<table width="100%" align="center" cellspacing="<?=$cell0?>" cellpadding="1" border="0" class="devicewidth" modulebg="edit" style="background:#fff">
								<tbody>
									<!--<tr>
					...
					<tr>
						<td>
							<table width="100%" align="center" cellspacing="<?=$cell1?>" cellpadding="0" border="0" class="devicewidth" modulebg="edit" style="background:#fff;">
								<tbody>
								<?php if($proposal_type!=5 && $proposal_type!=6 && $proposal_type!=7){ ?>
									<tr>
										<td style="text-align:left;font-size:12px;">Solpaneler: <?php $ids = getids($panel_model);
																	// print_r($ids);die;
								...
								<?php } ?>
									<?php if($charger_name){?>
									<tr>
										<td style="text-align:left;font-size:12px;">Billaddare: <?php $ids = getids($charger_name);
																	// print_r($ids);die;
																$charger_name = json_decode($ids['content'],true);
																foreach($charger_name as $cval){
																echo $cval['name']; }
															?></td>
										<td style="text-align:center;font-size:12px;">1</td>
										<td style="text-align:center;font-size:12px;">st</td>
									...
									<tr>
										<td style="text-align:left;font-size:12px;">Lastbalanserare: <?php $ids = getids($charger_name);
																	// print_r($ids);die;
																$charger_name = json_decode($ids['content'],true);
																foreach($charger_name as $cval){
																echo $cval['name']; }
									...
									<?php if($battery_name){?>
									<tr>
										<td style="text-align:left; font-size:12px;">Batteri: <?php $ids = getids($battery_name);
																	// print_r($ids);die;
																$battery_name = json_decode($ids['content'],true);
																foreach($battery_name as $cval){
																echo $cval['name']; }
															?></td>
										<td style="text-align:center; font-size:12px;">1</td>
										<td style="text-align:center; font-size:12px;">st</td>
							...
							<table width="100%" align="center" cellspacing="<?=$cellspacing?>" cellpadding="<?=$cellpadding?>" border="0" class="devicewidth" modulebg="edit" style="background:#fff">
								<tbody>
									<tr>
										...
										<td style="text-align:left; font-size:12px;">
											<?=amount_format_proposal(round($charger_price_excluding_vat))?> kr
										</td>
									</tr>
									<tr>
										<?php if($proposal_customer_type==2){ $bold = 'font-weight: bold;'; }
										...
										<td style="text-align:left; font-size:12px; <?=$bold?>">
											<?=amount_format_proposal(round($charger_price_including_vat))?> kr
										</td>
									</tr>
								</tbody>
							</table>
							...
							<table width="100%" align="center" cellspacing="<?=$cellspacing?>" cellpadding="<?=$cellpadding?>" border="0" class="devicewidth" modulebg="edit" style="background:#fff; border-top: 1px solid #000;">
								<tbody>
									<tr>
										...
										<td style="text-align:left; font-size:12px;">
											<b><?=amount_format_proposal(round($charger_price_after_green_deduction))?> kr</b>
										</td>
									</tr>
								</tbody>
							</table>
						...
						<td>
							<table width="100%" align="center" cellspacing="<?=$cellspacing?>" cellpadding="<?=$cellpadding?>" border="0" class="devicewidth" modulebg="edit" style="background:#fff">
								<tbody>
									<tr>
										<td style="text-align:left; font-size:12px;">
											Pris lastbalanserare exklusive moms:
										</td>
										<td style="text-align:left; font-size:12px;">
											<?=amount_format_proposal(round($load_price_excluding_vat))?> kr
										</td>
									</tr>
									<tr>
										<?php if($proposal_customer_type==2){ $bold = 'font-weight: bold;'; }
										...
										<td style="text-align:left; font-size:12px; <?=$bold?>">
											Pris lastbalanserare inklusive moms:
										</td>
										<td style="text-align:left; font-size:12px; <?=$bold?>">
											<?=amount_format_proposal(round($load_price_including_vat))?> kr
										</td>
									</tr>
								</tbody>
							</table>
						...
						<td>
							<table width="100%" align="center" cellspacing="<?=$cellspacing?>" cellpadding="<?=$cellpadding?>" border="0" class="devicewidth" modulebg="edit" style="background:#fff; border-top: 1px solid #000;">
								<tbody>
									<tr>
										<td style="text-align:left; font-size:12px;">
											<b>Pris lastbalanserare med grönt avdrag***:</b>
										</td>
										<td style="text-align:left; font-size:12px;">
											<b><?=amount_format_proposal(round($load_price_after_green_deduction))?> kr</b>
										</td>
									</tr>
								</tbody>
							</table>
					...
					<?php if($proposal_type==1 || $proposal_type==2 || $proposal_type==5){ ?>
					<tr><td></td></tr>
					<?php } ?>
					<?php 
					if($proposal_type==1){ 
						...
						<td>
							<table width="100%" align="center" cellspacing="<?=$textspacing?>" cellpadding="<?=$textpadding?>" border="0" class="devicewidth" modulebg="edit" style="background:#fff">
								<tbody>
									...
									<tr>
										<td style="text-align:left; font-size:12px;">**Grönt avdrag är 20% av totalbelopp menomfattar enbart material & arbetskostnader. Schablon är satt av Skatteverket till 19,4%. För mer information om bidraget och dess villkor, läs mer på <a href="https://www.skatteverket.se/">www.skatteverket.se</a></td>
									</tr>
										<?php } } ?>
									<?php if($charger_name!='' || $battery_name!=''){
										if($proposal_customer_type==1){?>
									<tr>
