...
<?php 
	if($action=='del'){
		$cms->db_query("delete from #_blog_catagories where id in ($id)");
	...
	if($cms->is_post_back()){
		if($arr_ids) {
			...
			switch ($_POST['action']){
				case "delete":
					$cms->db_query("delete from #_blog_catagories where id in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Delete', 'e');
					break;
				case "Inactive":
					$cms->db_query("update #_blog_catagories set status = '0' where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Inactive', 'e');
					break;
				case "Active":
					$cms->db_query("update #_blog_catagories set status = '1' where pid in ($str_adm_ids)");
	...
	if(isset($_REQUEST['submt'])){
		...
		$columns = "select * ";
		$sql = " from #_blog_catagories where 1=1 AND is_deleted=0 $conditionsQr ";
		...
		$pagesize = intval($pagesize)==0?$pagesize=DEF_PAGE_SIZE:$pagesize;
		$columns = "select * ";
		$sql = " from #_blog_catagories where 1=1 AND is_deleted=0 ";
		$order_by == '' ? $order_by = 'id' : true;
		$order_by2 == '' ? $order_by2 = 'desc' : true;
		$sql_count = "select count(*) ".$sql; 
		$sql .= "order by $order_by $order_by2 ";
