...
if($cms->is_post_back()){ 
	if($pid){ 
		...
		$cms->sqlquery("rs","blog_catagories",$_POST,'id',$pid);
		$adm->sessset('Record has been updated', 's');
	} else { 
		$_POST['modified'] = date("Y-m-d h:i:s");
		$_POST['cat_url'] = $cms->baseurl($_POST['cat_name']);
		$uids = $cms->sqlquery("rs","blog_catagories",$_POST);
...
if(isset($pid)){
	$rsAdmin=$cms->db_query("select * from #_blog_catagories where id='".$pid."'");
    ...
    <div class="col-sm-12">
        <div class="white-box">		
			<!--<div class="form-group col-sm-6">
                <label for="title" class="control-label">Parent Catagory*</label>
				<select class="form-control select2" name="parent_id" title="Parent Category">
					<?php //print "<option value=''>Select Parent Category</option>";
					//echo get_blog_category_list(0,$parent_id);
					?>
				</select>
