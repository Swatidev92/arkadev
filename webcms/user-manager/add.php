...
if(isset($_POST['send_mail'])){
	...
	if($emailExists>0  && $emailExists!=''){
		...
		$USERS['plain_password'] = $user_password;
		$USERS["password"] = md5($user_password);
		$USERS["email_id"] = $_POST['email_id'];
		$USERS["customer_name"] = $_POST['customer_name'];
		$USERS["contact_no"] = $_POST['contact_no'];
		$USERS["role"] = $_POST['role'];
				
		$user_id=$cms->sqlquery("rs","users",$USERS);
		...
		if($user_id){
			...
			$msg_body='<div>Hi,</div>
					   ...
					   <div>Password : '.$USERS['plain_password'].'</div>
					   <div>&nbsp;</div>
...
<div class="row">
    <div class="col-sm-12">
        <div class="white-box">
			...
			<div class="clearfix"></div>
            <div class="form-group col-sm-12">
                <button type="submit" class="btn btn-primary" name="save_user_data">Submit</button>
