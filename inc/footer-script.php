<?php $cookieQry=$cms->db_query("select * from #_cookie where id=1 ");
$cookieRes=$cookieQry->fetch_array(); 

$solarQry = $cms->db_query("SELECT `sec2_title1`, `sec2_title2`, `sec2_title3`, `sec2_title1_sw`, `sec2_title2_sw`, `sec2_title3_sw`, `sec2_panel1`, `sec2_panel2`, `sec2_panel3`, `sec2_panel1_sw`, `sec2_panel2_sw`, `sec2_panel3_sw`, `panel_capacity1`, `panel_capacity2`, `panel_capacity3`, `panel_capacity1_sw`, `panel_capacity2_sw`, `panel_capacity3_sw` FROM #_solar where id=1 ");

$solarRes = $solarQry->fetch_array();
											
?>
 
 <div class="modal fade" id="cookie-poup" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-body">
				<div class="container" style="width:auto;">
					<h2 class="css-1xug45r"><?=$countryConst=='SE'?'Hantera inställningar för samtycke':'Manage your choices'?></h2>
					<hr class="css-pv5xwe">
					<div class="row">
						<div class="col-md-2">			
							<label class="switch">
						...
						<div class="col-md-6">
							<p class="cookie-options"><?=$countryConst=='SE'?'Nödvändig':'Necessary'?></p>
						</div>
						
						<div class="col-md-12">			
							<p class="cookie-options-content"><?=$cookieRes['necessary_content'.$langf]?></p>
						</div>
					</div>
					<div class="row">
						<div class="col-md-2">			
						...
						<div class="col-md-6">
							<p class="cookie-options"><?=$countryConst=='SE'?'Statistik':'Statistics'?></p>
						</div>
						
						<div class="col-md-12">			
							<p class="cookie-options-content"><?=$cookieRes['statistics_content'.$langf]?></p>
						</div>
					</div>	
					...
					<div class="row">
						...
						<div class="col-md-6">
							<p class="cookie-options"><?=$countryConst=='SE'?'Marknadsföring':'Marketing'?></p>
						</div>
						
						<div class="col-md-12">			
							<p class="cookie-options-content"><?=$cookieRes['marketing_content'.$langf]?></p>
						</div>			
					</div>
				</div>
			</div>
				
			<div class="css-5r26ll"><a id="cookieBoxokpop" class="cookieok" data-cookie="risk"><?=$countryConst=='SE'?'Godkänn val':'Accept Choices'?></a></div>
		</div>
	</div>
</div>
 ...
 <div id="cookiebarBox" class="os-animation" data-os-animation="fadeIn" >
    <div class="container" >
		<div class="cookieAcceptance">
			...
			<div class="cookieok-box"><a href="#" data-toggle="modal" data-target="cookie-poup" class="manage-cookie">Hantera dina val</a> <a id="cookieBoxok" class="cookieok" data-cookie="risk">Ok</a></div>
			<?php }else{ ?>		
			...
			<div class="cookieok-box"><a href="#" data-toggle="modal" data-target="cookie-poup" class="manage-cookie">Manage your choices</a> <a id="cookieBoxok" class="cookieok" data-cookie="risk">Ok</a></div>		
			<?php } ?>
		</div>
    </div>
</div>
...
jQuery(document).ready(function($) {
  ...
  var closeCookieBoxPopup = document.getElementById("cookieBoxokpop");
    closeCookieBox.onclick = function() {
        cookieBox.style.display = "none";
    };
	closeCookieBoxPopup.onclick = function() {
        closeCookieBoxPopup.style.display = "none";
    };
});
...
(function () {
	...
	$(document).ready(function(){ 
		$("input[name$='panel_type']").click(function() {
			...
			if(selval == 'Optimized'){
				$('#panel_val').val(<?=$solarRes['panel_capacity1'.$langf]?>);
				if(nameArr[0]=='at1' && typeof(nameArr[1]) == "undefined"){
					$('.essential-img').attr('src', '<?=SITE_PATH?>assets/images/calculator/panels/optimized-battery.jpg');
				}
				else if(nameArr[0]=='at2' && typeof(nameArr[1]) == "undefined"){
					$('.essential-img').attr('src', '<?=SITE_PATH?>assets/images/calculator/panels/optimized-charger.jpg');
				}
				else if(nameArr[0]=='at1' && nameArr[1]=='at2'){
					$('.essential-img').attr('src', '<?=SITE_PATH?>assets/images/calculator/panels/optimized-both.jpg');
				}else{
					$('.essential-img').attr('src', '<?=SITE_PATH?>assets/images/calculator/panels/optimized.jpg');
				}				
				$('#onPanelSel1').show();
				$('#onPanelSel2').hide();
				$('#onPanelSel3').hide();
			}else if(selval == 'Esthetic'){
				$('#panel_val').val(<?=$solarRes['panel_capacity2'.$langf]?>);
				if(nameArr[0]=='at1' && typeof(nameArr[1]) == "undefined"){
					$('.essential-img').attr('src', '<?=SITE_PATH?>assets/images/calculator/panels/esthetic-battery.jpg');
				}
				else if(nameArr[0]=='at2' && typeof(nameArr[1]) == "undefined"){
					$('.essential-img').attr('src', '<?=SITE_PATH?>assets/images/calculator/panels/esthetic-charger.jpg');
				}
				else if(nameArr[0]=='at1' && nameArr[1]=='at2'){
					$('.essential-img').attr('src', '<?=SITE_PATH?>assets/images/calculator/panels/esthetic-both.jpg');
				}else{
					$('.essential-img').attr('src', '<?=SITE_PATH?>assets/images/calculator/panels/esthetic.jpg');
				}
				$('#onPanelSel1').hide();
				$('#onPanelSel2').show();
				$('#onPanelSel3').hide();
			}else if(selval == 'Performance'){
				$('#panel_val').val(<?=$solarRes['panel_capacity3'.$langf]?>);	
				if(nameArr[0]=='at1' && typeof(nameArr[1]) == "undefined"){
					$('.essential-img').attr('src', '<?=SITE_PATH?>assets/images/calculator/panels/esthetic-battery.jpg');
				}
				else if(nameArr[0]=='at2' && typeof(nameArr[1]) == "undefined"){
...
function hasScrolled() {
    ...
    if (st > lastScrollTop && st > navbarHeight){
        ...
        $('.page-head-banner').removeClass('page-head-banner-fixed');
    } else {
        // Scroll Up
        if(st + $(window).height() < $(document).height()) {
            $('.transparent-menu').addClass('menu_fixed');
            $('.calculator-padding').addClass('calc_fixed');
            $('.page-head-banner').addClass('page-head-banner-fixed');
        }
    }
	...
	$(document).ready(function () {
		...
		$("#step4_back").click(function () {			
			$('.annual-cost-count').html('-');
			$('.annual-saving-count').html('-');
		});
		...
		*/
		...
		$("#back-to-address").click(function (e) {
			$('.more-panel').html('');
			$('.contact-link-map').html('');
			$('#step2_submit').show();
			$('#roof_area').val('');
		...
		if($("#"+btnid).hasClass("disabled")==false){
			$.ajax({
				...
				success:function(result){
					...
					//alert(result);
					if(result==1){	
						location.href="<?=SITE_PATH.$urlConst?>thankyou";
					}else{	
						$(".amsg").html('<div class="alert alert-danger" role="alert"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Went wrong</div>');
					}
				}
		...
		if($("#"+btnid).hasClass("disabled")==false){
			$.ajax({
				...
				success:function(result){
					...
					if(res==-1){
						$("."+msg).html('<div class="alert-invalid-captcha" style="color:red">Invalid Captcha.Please try again.</div>');
						return false;
					}else if(res==1){
						//location.href="<?=SITE_PATH?>thankyou";
						...
						$("."+msg).html('<div class="alert alert-danger alert-dismissible" style="color:red"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Something went wrong. Please try again.</div>');
					}
				}
			});
		}
